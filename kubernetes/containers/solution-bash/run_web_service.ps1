...
$restarts_before = (kubectl get po web -o jsonpath='{.status.containerStatuses[0].restartCount}') -replace '"'
...
$restarts_after = (kubectl get po web -o jsonpath='{.status.containerStatuses[0].restartCount}') -replace '"'

# Verify again the web service is running
kubectl exec web -- ps

# Print the results
Write-Host "Restarts before killing the process: $restarts_before"
Write-Host "Restarts after killing the process: $restarts_after"